...
| [`CLOUD-025`](CLOUD-025-browser-session-logout-protection.md) | Bind user and developer browser logout to the configured same-origin JSON boundary | `verification_pending` — focused tests, builds, and local credentialed split-origin browser flow pass; standing aggregate preflight and hosted readback remain |
