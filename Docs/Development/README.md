...
| [`DOCS-006`](DOCS-006-bound-task-product-restoration.md) | Restore existing-task, notification-only product authority and preserve exact compatibility evidence | Task-scoped documentation locally verified; full-repository sensitive scan not clean; runtime implementation open |
| [`DOCS-007`](DOCS-007-judge-facing-readme.md) | Reframe the root README around the demo, user-governed continuity, business potential, and clearly aspirational native Agent integration | `locally_verified` documentation; no runtime or deployment change |
...
| [`CLOUD-023`](CLOUD-023-standing-receiver-source-gate.md) | Additive standing Receiver kernel and fail-closed committed-source preflight | Core/Receiver exact pin refreshed; source fixtures `16/16`, pinned real-store trace `1/1` now includes shared out-of-order rejection/no-mutation, concurrent duplicate convergence, the distinct-Event sequence race, same-ID identity conflict, and one-shot post-write rollback with exact-envelope retry; bounded Core/SQLite and active Receiver/PostgreSQL fresh-process and transaction-interruption fixtures also pass; active-v2 delivery profile `1/1` passes; the prior backend aggregate `21/21` suites / `158` tests remains green locally; full release conformance and public controls remain open |
| [`CLOUD-024`](CLOUD-024-connector-browser-capability-preflight.md) | Verify the current fresh CLI Browser capability separately from desktop access | Fresh CLI selects Chrome extension with zero tabs; authenticated Game read blocked pending session-preserving handoff; production route unproved |
| [`CLOUD-025`](CLOUD-025-browser-session-logout-protection.md) | Bind user and developer browser logout to the configured same-origin JSON boundary | `verification_pending` — focused tests, builds, local credentialed split-origin browser flow, and backend aggregate (`21/21` suites, `158` tests) pass; hosted readback remains |
