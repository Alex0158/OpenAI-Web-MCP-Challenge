...
| [`CLOUD-023`](CLOUD-023-standing-receiver-source-gate.md) | Additive standing Receiver kernel and fail-closed committed-source preflight | Core/Receiver exact pin refreshed; source fixtures `16/16`, migration guards `5/5`, pinned real-store trace `1/1` now includes shared out-of-order rejection/no-mutation, concurrent duplicate convergence, the distinct-Event sequence race, and same-ID identity conflict; active-v2 delivery profile `1/1` also passes; the prior backend aggregate `21/21` suites / `158` tests remains green locally; full release conformance and public controls remain open |
