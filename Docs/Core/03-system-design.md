...
**Status:** Canonical Re-entry Core architecture under ADR-0006; ADR-0007 fixes the v0.1 protocol, ADR-0008 fixes Receiver consent, Grant creation, reservation, and pending-delivery authority, ADR-0009 fixes the locally verified Connector lease and effect-acknowledgement contract, ADR-0010 fixes the locally verified HTTP adapter, outbound Connector client, and forced-restart test-process isolation, ADR-0011 fixes the locally verified deterministic Agent activation boundary, and ADR-0012 fixes the locally verified non-production conformance/development profile. Receiver-owned Grant inspection/revocation, private context-binding lifecycle, production process shells, and the concrete Agent continuation adapter remain open. Current as-built truth is owned by Core/00 and Core/05.  
...
ADR-0012 fixes one locally verified source-repository conformance profile outside `test/`. Distinct Host,
...
- ADR-0012 direct runs and aggregate tests locally verify a non-production conformance/development
  ...
  acknowledgement before a separate synthetic Host effect, emits one redacted result, and cleans
  exact temporary state. It is not a production process shell or runtime package export.
