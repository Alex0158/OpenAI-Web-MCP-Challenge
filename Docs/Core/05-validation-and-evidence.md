...
| Current host-mediated task control | **B1 JOINED-TURN RESPONSE VERIFIED; D1 ISOLATED IDLE CONTROL FAILED** | CLOUD-028: B1 produced an exact tool-data response after older Q1. D1 used an idle disposable target with no queue or goal but failed waiting for an active turn ID; no new target input/turn/response was observed. B1/D1 consumed, no resend; C1 unused. App-side pending-state cause unresolved; no independent Connector, Browser or notification-receipt proof |
