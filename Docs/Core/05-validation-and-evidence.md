...
**Status:** Current proof matrix plus future evidence gates; the Re-entry Core v0.1 protocol, Host SDK, Receiver authority, Connector delivery, transport and bounded process isolation, deterministic Agent Adapter contract, non-production conformance profile, local quality and weight baseline, Program audit, Receiver Grant control, and exact RECORE-005 test-process fault matrix are locally verified. Private context binding, arbitrary crash or power-loss safety, production process ownership, service performance, and selected-app budgets remain open; bounded P0/H1/H2 reference evidence passed, D4 is inconclusive, and production consent, pairing, control-session security, a real Host-effect verifier, a supported Agent adapter, app, product, and judge gates remain open.  
...
| Authoritative `reentry-core/` implementation | PARTIALLY LOCALLY VERIFIED | Protocol, Host SDK, Receiver authority and Grant control, Connector delivery state machine, HTTP mapping, outbound client, test-process isolation, deterministic Agent Adapter contract, non-production conformance profile, SQLite reference store, and local regression baselines pass bounded checks; private context binding, production consent and pairing, process shells, real Host effect, Connector daemon, concrete adapter, deployment, service performance, and selected-app budgets remain open |
...
| App-independent quality and weight baseline | LOCALLY VERIFIED, NON-SLA | RECORE-002 runs bounded protocol, deterministic Agent Adapter, file-backed Receiver, and cold source-profile benchmarks on Node 24.20.0 and Node 26.5.0. Zero runtime dependencies and the 15-file package allowlist remain intact; benchmark, conformance, and test sources are excluded. Results are same-machine regression samples, not hosted throughput, production latency, capacity, version ranking, or an app budget |
| Program completion audit | LOCALLY VERIFIED; PROGRAM ACTIVE | RECORE-003 maps each Definition of Done item to direct evidence and records RECORE-004 and RECORE-005 as post-audit progress. Private context binding and final exact closure remain incomplete |
| Receiver-authenticated Grant control | LOCALLY VERIFIED IN CORE AND BOUNDED TEST PROCESSES | ADR-0013 plus RECORE-004; 36 focused Receiver/store tests and 67 aggregate tests pass on Node 24.20.0 and Node 26.5.0. RECORE-005 then verifies exact revocation/restart/race behavior across test processes. Runtime dependencies remain zero and the 15-file package boundary remains intact. No control HTTP, UI/session, anti-CSRF, production identity, or private context binding is proven |
