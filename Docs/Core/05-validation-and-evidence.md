...
**Status:** Current proof matrix plus future evidence gates; the Re-entry Core v0.1 protocol, Host SDK, ADR-0008 Receiver C1 authority, ADR-0009 Connector delivery C2, and ADR-0010 transport plus test-process isolation are locally verified; forced termination and production process ownership remain open; bounded P0/H1/H2 reference evidence passed, D4 is inconclusive, and production consent, pairing, a real Host-effect verifier, a supported Agent adapter, app, product, and judge gates remain open.  
...
| Receiver HTTP and outbound Connector process boundary | TRANSPORT AND TEST-PROCESS ISOLATION LOCALLY VERIFIED | ADR-0010 plus RECORE-001 C3c; 48 aggregate tests pass on Node 24.20.0 and Node 26.5.0. Independent Host, Receiver, and Connector children prove signed HTTP event acceptance, file-backed event and claim replay across restart, effect-token rejection, acknowledgement-response-loss convergence, Receiver-only SQLite ownership, and raw-token absence. Production shells, forced termination, TLS, pairing, real effects, and Agent behavior remain required |
