...
| Active Cloud Receiver v2 | **LOCAL COMMITTED-SOURCE UPGRADE AND MINIMUM PINNED TRACE VERIFIED; RELEASE OPEN** | Historical v0.1/backend and preview evidence remain bounded separately; the locally committed `Re-Entry` standing increment passes 156 backend tests, type-check/build, exact-commit six-to-seven disposable-PostgreSQL upgrade with unchanged old rows/catalog, and a two-signal Express trace against reviewed committed Core source. Consent/control use internal seams and effect authority is deterministic. Full pinned v0.1/v0.2 conformance, rollback/crash recovery, release enforcement, public controls, lifetime, exact deployment, and full product flow remain open under TASK-028/TASK-033; CLOUD-023 and the Receiver record own exact identities |
