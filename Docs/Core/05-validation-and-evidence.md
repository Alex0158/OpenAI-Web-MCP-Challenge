...
| Standing Core fresh-process recovery | **LOCALLY VERIFIED, CORE/SQLITE ONLY** | one committed pending Delivery survives SIGKILL, a new process claims and acknowledges it, and exact Event replay converges; other implementation boundaries and supervision remain open |
| Active Receiver fresh-process recovery | **LOCALLY VERIFIED, ACTIVE RECEIVER/POSTGRESQL ONLY** | one committed pending Delivery survives SIGKILL across active Express/Prisma processes and exact Event replay converges; production authority, supervision, distributed ownership, and release recovery remain open |
| Active Receiver transaction interruption | **LOCALLY VERIFIED, ACTIVE RECEIVER/POSTGRESQL ONLY** | a test-only kill after Delivery write rolls back the Grant/Event/Delivery transaction, exact Event retry succeeds, and committed Delivery recovery converges; production failure handling and release recovery remain open |
| Standing Core transaction interruption | **LOCALLY VERIFIED, CORE/SQLITE ONLY** | a test-only kill after Delivery write rolls back the Grant/Event/Delivery transaction, exact Event retry succeeds, and committed Delivery recovery converges; production durability and release recovery remain open |
...
| Active v2 pairing claim abuse fence | **LOCALLY AND HOSTED PREVIEW VERIFIED; PRODUCTION OPEN** | strict pairing ID/code shape, atomic attempt boundary, durable 30-per-10-minute source bucket, fixed-window reset, and bounded `429` across the exact Re-Entry Vercel Preview; Production secret custody, managed migration, promotion, and full deployed flow remain open |
...
| Account-first consent and background Connector path | **CLOUD RECEIVER RUNTIME DEPRECATED; SDK/CONNECTOR SEAMS RETAINED AS HISTORICAL PREVIEW EVIDENCE** | dashboard-issued pairing ID/code, credential reuse, organization-key Host control, Re-entry-owned consent and device selection, bounded polling, LaunchAgent profile, and fresh Codex process seam; no production identity, deployment, or supported Browser/WebMCP join |
...
| Active Cloud Receiver v2 | **LOCAL COMMITTED-SOURCE UPGRADE, MINIMUM PINNED TRACE, AND BOUNDED RECOVERY VECTORS VERIFIED; RELEASE OPEN** | Historical v0.1/backend and preview evidence remain bounded separately; the locally committed `Re-Entry` standing increment passes the prior 158-test backend aggregate, type-check/build, exact-commit six-to-seven disposable-PostgreSQL upgrade with unchanged old rows/catalog, refreshed source-pin fixtures `16/16`, migration guards `5/5`, the pinned two-signal Express trace, and separate fresh-process and transaction-interruption fixtures. Consent/control use internal seams and effect authority is deterministic. Full pinned v0.1/v0.2 conformance, remaining rollback/crash recovery, release enforcement, public controls, lifetime, exact deployment, and full product flow remain open under TASK-028/TASK-033; CLOUD-023 and the Receiver record own exact identities |
| Simple SDK-to-v2-to-Connector composition | **SEPARATE-PROCESS VERIFIED, NON-PRODUCTION** | Core 81/81, SDK 25/25, Connector 49 executed (12 opt-in active-v2 tests skipped), one separate Connector worker plus separate test effect/ack worker and Receiver restart replay; default product effect authority and supported external Agent/Browser absent |
...
| Frozen MVP1 technical composition | **REAL SAME-TASK / BROWSER / WEBMCP VERIFIED ON RECORDED BUILD; IDLE WAKE NOT ISOLATED** | P0 Stage-B and rehearsal preserve prior task context and genuinely continue the artifact. CLOUD-028's original invocation readback shows goal auto-continuation had already started the target turn before each Event; neither run proves Event-caused idle wake or current product Connector integration |
| Current host-mediated task control | **D2 CLEAN IDLE WAKE RUNTIME-VERIFIED; D1 OLD-TARGET FAULT UNRESOLVED** | CLOUD-028: D2 verified idle/no-goal/no-queue/no-automation, then one App-tool notification started a distinct same-task turn and exact marker response with no tools. Warm-state start succeeds, disproving a blanket idle/warm-state restriction. D1's orphaned optimistic-state cause remains source/model-supported, not captured or fixed. B1/D1/D2 consumed, C1 unused; no independent Connector, repeated-handoff reliability, Browser or notification-receipt proof |
| Public CLI same-task wake | **Q2 LOADED-IDLE WAKE RUNTIME-VERIFIED; PRODUCT INTEGRATION OPEN** | CLOUD-028: one public `codex queue` to the existing loaded-idle fixture starts a distinct turn and returns its exact marker with zero tools. Passive observation precedes post-send App readback; no App-message rescue. Actual input is `userMessage`. Q2 consumed; no durable binding, unloaded/restart guarantee, product Connector consumer, qualified handoff receipt or Browser/WebMCP proof |
...
| Supported concrete Agent-to-Browser/WebMCP join | **CURRENT PRODUCT INTEGRATION UNVERIFIED** | Original MVP1 success and current host-mediated task response remain valid at their stated boundaries. CLOUD-024 is fresh CLI preview evidence only. TASK-035/TASK-034 still require actual Connector invocation, trusted binding, attributable wake and authenticated Game/WebMCP return; no production driver selected |
