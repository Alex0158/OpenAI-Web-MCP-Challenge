...
| Authoritative `reentry-core/` implementation | PARTIALLY LOCALLY VERIFIED | Protocol, Host SDK, Receiver authority, Connector delivery state machine, HTTP mapping, outbound client, test-process isolation, deterministic Agent Adapter contract, non-production conformance profile, SQLite reference store, and local regression baselines pass bounded checks; production consent and pairing, process shells, real Host effect, Connector daemon, concrete adapter, deployment, service performance, and selected-app budgets remain open |
...
| Program completion audit | LOCALLY VERIFIED; PROGRAM ACTIVE | RECORE-003 maps each Definition of Done item to direct evidence. Existing revocation tests inject `revoked_at` through read overrides and therefore prove rejection semantics only, not a revocation write lifecycle. Private context binding, the remaining separate-process fault matrix, and final exact closure remain incomplete |
| Receiver-authenticated Grant control | SPECIFIED; NOT IMPLEMENTED | ADR-0013 freezes exact same-subject inspection, revocation, replay, and race semantics through the existing `revoked_at` state. RECORE-004 implementation and direct evidence remain required |
