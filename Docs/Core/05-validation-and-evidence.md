...
**Status:** Current proof matrix plus future evidence gates; the Re-entry Core v0.1 protocol, Host SDK, Receiver authority, Connector delivery, transport and bounded process isolation, deterministic Agent Adapter and private binding-resolution contracts, non-production conformance profile, local quality and weight baseline, Program audit, Receiver Grant control, and exact RECORE-005 test-process fault matrix are locally verified. Final exact Program closure remains under audit; arbitrary crash or power-loss safety, production process ownership, binding capture and custody, service performance, and selected-app budgets remain open. Bounded P0/H1/H2 reference evidence passed, D4 is inconclusive, and production consent, pairing, control-session security, a real Host-effect verifier, a supported Agent adapter, app, product, and judge gates remain open.  
...
| Authoritative `reentry-core/` implementation | APPLICATION-NEUTRAL CONTRACTS LOCALLY VERIFIED; FINAL AUDIT OPEN | Protocol, Host SDK, Receiver authority and Grant control, Connector delivery state machine, HTTP mapping, outbound client, test-process isolation, deterministic Agent Adapter and private binding-resolution contracts, non-production conformance profile, SQLite reference store, and local regression baselines pass bounded checks. Production consent and pairing, process shells, binding custody, real Host effect, Connector daemon, concrete adapter, deployment, service performance, and selected-app budgets remain decision-gated |
...
| Receiver HTTP, outbound Connector, and process-fault boundary | TRANSPORT AND EXACT FAULT-MATRIX TEST ISOLATION LOCALLY VERIFIED | ADR-0010, RECORE-001 C3d, and RECORE-005; 71 aggregate tests and 5 focused process tests pass on Node 24.20.0 and Node 26.5.0. Independent Host, Receiver, and Connector children prove restart replay, acknowledgement-response-loss recovery, revocation-before-event, lease-before-revocation convergence, expired-lease stale-worker fencing, exact and conflicting effect behavior, one exact pre-commit `SIGKILL` rollback point, Receiver-only SQLite ownership, and bounded raw-token absence. Arbitrary crash placement, power loss, concurrent ownership, production shells, TLS, pairing, real effects, and Agent behavior remain unproved |
| Agent Adapter activation boundary | LOCALLY VERIFIED WITH DETERMINISTIC ADAPTER | ADR-0011 plus RECORE-001 C4b; the focused suite verifies credential-free activation, exact correlation, all result classes, all unavailable capabilities, timeout, exception, malformed result, one-call behavior, and no retry or acknowledgement. RECORE-006 reuses this boundary without changing its result shape. No real Agent, Browser, WebMCP, or Host effect is proven |
| Private managed-context binding resolution | LOCALLY VERIFIED WITH DETERMINISTIC AUTHORITY | ADR-0014 plus RECORE-006; 8 focused managed-context tests, 14 combined adapter tests, and 79 aggregate tests pass on Node 24.20.0 and Node 26.5.0. Exact Grant-only lookup, adapter scope, lifetime fencing, late resolution, missing and failure classifications, one driver call, and raw-reference non-disclosure are covered. No capture, production persistence or custody, retirement propagation, or real activation is proven |
...
| App-independent quality and weight baseline | LOCALLY VERIFIED, NON-SLA | RECORE-002 runs bounded protocol, deterministic Agent Adapter, file-backed Receiver, and cold source-profile benchmarks on Node 24.20.0 and Node 26.5.0. RECORE-006 rechecks the unchanged Agent benchmark and exact package boundary: zero runtime dependencies, 16 selected files, 34,227 compressed bytes, and 180,301 unpacked bytes; benchmark, conformance, and test sources are excluded. Results are same-machine regression samples, not hosted throughput, production latency, capacity, version ranking, or an app budget |
| Program completion audit | LOCALLY VERIFIED; FINAL EXACT AUDIT OPEN | RECORE-003 maps each Definition of Done item to direct evidence. RECORE-004, RECORE-005, and RECORE-006 close its three application-neutral gaps; the ledger and canonical surfaces still require the terminal reconciliation pass before Program closure |
| Receiver-authenticated Grant control | LOCALLY VERIFIED IN CORE AND BOUNDED TEST PROCESSES | ADR-0013 plus RECORE-004; 36 focused Receiver/store tests and 67 aggregate tests pass on Node 24.20.0 and Node 26.5.0. RECORE-005 then verifies exact revocation/restart/race behavior across test processes. Runtime dependencies remain zero. No control HTTP, UI/session, anti-CSRF, or production identity is proven; private binding resolution is a separate ADR-0014 boundary |
