...
**Status:** Current proof matrix plus future evidence gates; the Re-entry Core v0.1 protocol, Host SDK, ADR-0008 Receiver C1 authority, ADR-0009 Connector delivery C2, and ADR-0010 HTTP adapter and outbound Connector client are locally verified; separate-process transport remains open; bounded P0/H1/H2 reference evidence passed, D4 is inconclusive, and production consent, pairing, a real Host-effect verifier, a supported Agent adapter, app, product, and judge gates remain open.  
...
| Receiver HTTP and outbound Connector process boundary | ADAPTER AND CLIENT LOCALLY VERIFIED; PROCESS PROOF OPEN | ADR-0010 plus RECORE-001 C3b; 47 aggregate tests pass on Node 24.20.0 and Node 26.5.0. Focused tests cover exact event, claim, no-work, and acknowledgement mapping; bounds and redaction; secure origins; redirect rejection; timeout; one observed request; interrupted streams; and malformed, stale, oversized, or mismatched response rejection. Independent processes and restart remain required |
