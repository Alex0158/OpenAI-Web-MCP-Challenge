...
| Active Receiver fresh-process recovery | **LOCALLY VERIFIED, ACTIVE RECEIVER/POSTGRESQL ONLY** | one committed pending Delivery survives SIGKILL across active Express/Prisma processes and exact Event replay converges; production authority, transaction-interruption, supervision, and release recovery remain open |
...
| Active Cloud Receiver v2 | **LOCAL COMMITTED-SOURCE UPGRADE AND MINIMUM PINNED TRACE VERIFIED; RELEASE OPEN** | Historical v0.1/backend and preview evidence remain bounded separately; the locally committed `Re-Entry` standing increment passes the prior 158-test backend aggregate, type-check/build, exact-commit six-to-seven disposable-PostgreSQL upgrade with unchanged old rows/catalog, refreshed source-pin fixtures `16/16`, migration guards `5/5`, and the pinned two-signal Express trace now includes shared out-of-order rejection/no-mutation and concurrent duplicate convergence at Core `68a306e` / Receiver `fa5de9d`. Consent/control use internal seams and effect authority is deterministic. Full pinned v0.1/v0.2 conformance, rollback/crash recovery, release enforcement, public controls, lifetime, exact deployment, and full product flow remain open under TASK-028/TASK-033; CLOUD-023 and the Receiver record own exact identities |
