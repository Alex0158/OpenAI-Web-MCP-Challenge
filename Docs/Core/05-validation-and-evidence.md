...
**Status:** Current proof matrix plus future evidence gates; the Re-entry Core v0.1 protocol, Host SDK, ADR-0008 Receiver C1 authority, ADR-0009 Connector delivery C2, ADR-0010 transport plus forced-restart test-process isolation, ADR-0011 deterministic Agent Adapter contract, ADR-0012 non-production conformance/development profile, RECORE-002 local quality and weight baseline, RECORE-003 Program completion audit, and ADR-0013/RECORE-004 in-process Receiver Grant control are locally verified. Private context binding, mid-transaction crash injection, the remaining separate-process revocation, stale-worker, and conflicting-effect matrix, production process ownership, service performance, and selected-app budgets remain open; bounded P0/H1/H2 reference evidence passed, D4 is inconclusive, and production consent, pairing, control-session security, a real Host-effect verifier, a supported Agent adapter, app, product, and judge gates remain open.  
...
| Program completion audit | LOCALLY VERIFIED; PROGRAM ACTIVE | RECORE-003 maps each Definition of Done item to direct evidence and records RECORE-004 as post-audit progress. Private context binding, the remaining separate-process fault matrix, and final exact closure remain incomplete |
| Receiver-authenticated Grant control | LOCALLY VERIFIED, IN-PROCESS | ADR-0013 plus RECORE-004; 36 focused Receiver/store tests and 67 aggregate tests pass on Node 24.20.0 and Node 26.5.0. Direct evidence covers authority-before-lookup, exact private output, same-subject scope, Core-timestamped compare-and-set, replay, real persisted event/delivery fencing, effect-time ordering, rollback, inconsistent-write failure, file reopen, and raw-token non-persistence. Runtime dependencies remain zero and the 15-file package boundary remains intact. No control HTTP, UI/session, anti-CSRF, process separation, production identity, or private context binding is proven |
