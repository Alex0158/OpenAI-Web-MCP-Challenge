...
**Status:** Target Re-entry Core trust and reliability baseline under ADR-0006 through ADR-0010; the v0.1 protocol, Host SDK, bounded Receiver C1 authority, Connector delivery C2, HTTP adapter, outbound Connector client, and forced-restart test-process isolation are locally verified; mid-transaction crash injection, production process ownership, consent and pairing, real Host-effect verification, and distributed topology remain unverified.  
