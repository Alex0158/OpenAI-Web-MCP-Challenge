...
**Phase:** Re-entry Core foundation is active under ADR-0006. The v0.1 protocol, Host SDK, ADR-0008 Receiver C1 authority, and ADR-0009 Connector delivery C2 are locally verified on Node 24 and the current Node 26 runtime. C2 adds target-scoped replayable claims, short leases, per-delivery attempt bounds, stale-worker fencing, Host-effect-backed acknowledgement, additive schema migration, rollback, and file close-and-reopen persistence. ADR-0010 now specifies the minimal Host-event HTTP ingress and outbound Connector transport; its implementation remains open alongside production consent or pairing, separate processes, a real Host-effect verifier, a supported Agent adapter, final-app selection, deployment, product value, and judge portability. Bounded mechanism feasibility remains passed through P0, H1, H2a, and the synthetic H2 service contract, while both standalone App Server Desktop-Browser joins remain failed in the current build.  
...
| Receiver HTTP and outbound Connector transport | **SPECIFIED; NOT IMPLEMENTED** | ADR-0010 fixes three strict bounded JSON routes, canonical responses, redacted errors, an outbound-only no-retry Connector client, explicit package subpaths, and independent Host/Receiver/Connector process evidence. No transport code or process proof has passed yet |
...
| Re-entry Core transport C3a | **SPECIFIED ONLY** | ADR-0010 defines Host event, Connector claim, and acknowledgement HTTP mappings plus a no-retry outbound client. No HTTP, child-process, timeout, restart, TLS, pairing, Host-effect, or Agent behavior has passed yet |
...
| Challenge implementation | **RE-ENTRY CORE FOUNDATION IN PROGRESS; C1/C2 LOCALLY VERIFIED; C3A SPECIFIED** | Protocol, Host SDK, bounded Receiver authority, lease, and effect-acknowledgement semantics are locally verified; the transport contract is specified, while separate services, Agent activation, selected app, deployment, demo, and submission closure remain open |
...
3. Complete [RECORE-001](../Development/RECORE-001-foundation.md) in bounded increments. The
   protocol, Host SDK, ADR-0008 Receiver authority, and ADR-0009 Connector delivery contract are
   locally verified. ADR-0010 now fixes the minimal wire boundary; implement and prove its
   separate Host, Receiver, and Connector processes without changing Core authority or modifying
   MVP1 or MVP2.
