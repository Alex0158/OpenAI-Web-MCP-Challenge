...
**Phase:** RECORE-001, RECORE-002, and the RECORE-003 Program completion audit are locally verified, and RECORE-004 is specified under accepted ADR-0013. The broader Re-entry Core program remains active. The v0.1 protocol, Host SDK, Receiver C1 authority, Connector delivery C2, transport and forced-restart test-process isolation C3d, deterministic Agent Adapter C4b, source-repository conformance/development profile C6b, and bounded local quality baseline pass on Node 24 and the current Node 26 runtime. ADR-0013 now freezes the missing Receiver-authenticated Grant inspection/revocation contract without claiming implementation. Private managed-context binding and the separate-process revocation, stale-worker, conflicting-effect, and mid-transaction failure matrix remain application-neutral gaps. Production ownership, process shells, consent or pairing, credential custody, a real Host-effect verifier, a supported Agent adapter, final-app selection, deployment, product value, and judge portability remain decision-gated or open outside those bounded increments. Bounded mechanism feasibility remains passed through P0, H1, H2a, and the synthetic H2 service contract, while both standalone App Server Desktop-Browser joins remain failed in the current build.  
...
| Re-entry Core Program completion audit | **LOCALLY VERIFIED; PROGRAM ACTIVE** | RECORE-003 maps every Definition of Done item to direct current evidence. Grant inspection/revocation, private managed-context binding, the remaining separate-process fault matrix, and final exact closure remain incomplete; production and selected-app work is not substituted for those Core gaps |
| Receiver Grant control | **SPECIFIED; NOT IMPLEMENTED** | ADR-0013 freezes one Receiver-authenticated same-subject inspection/revocation port, exact private outputs, existing-column compare-and-set, and event/delivery ordering. RECORE-004 must implement and verify it before any status upgrade |
...
3. Preserve [RECORE-001](../Development/RECORE-001-foundation.md) as the locally verified
   ...
   ledger. Implement [RECORE-004](../Development/RECORE-004-grant-control.md) under ADR-0013
   without combining it with private context binding or administration HTTP. Keep
