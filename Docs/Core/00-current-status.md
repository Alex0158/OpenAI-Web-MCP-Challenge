...
**Phase:** Re-entry Core foundation is active under ADR-0006. The v0.1 protocol, Host SDK, ADR-0008 Receiver C1 authority, ADR-0009 Connector delivery C2, and ADR-0010 transport and test-process isolation C3c are locally verified on Node 24 and the current Node 26 runtime. C3c proves independent Host, Receiver, and Connector child processes over the unchanged HTTP and client subpaths, including file-backed restart replay and acknowledgement-response-loss convergence, without adding runtime process routes or dependencies. Forced process termination, concurrent Receiver ownership, production consent or pairing, a real Host-effect verifier, a supported Agent adapter, final-app selection, deployment, product value, and judge portability remain open. Bounded mechanism feasibility remains passed through P0, H1, H2a, and the synthetic H2 service contract, while both standalone App Server Desktop-Browser joins remain failed in the current build.  
...
| Receiver HTTP and outbound Connector transport | **TRANSPORT AND TEST-PROCESS ISOLATION LOCALLY VERIFIED** | ADR-0010 and RECORE-001 C3c verify three strict bounded JSON routes, a no-retry outbound client, and independent Host/Receiver/Connector child processes. The Host alone owns its private key, the Receiver alone opens SQLite, and the Connector uses only HTTP. Production shells, TLS, pairing, real effects, and Agent behavior remain unproved |
...
| Re-entry Core transport C3 | **TRANSPORT AND TEST-PROCESS ISOLATION LOCALLY VERIFIED** | ADR-0010 and RECORE-001 C3c pass 48 aggregate tests on Node 24.20.0 and Node 26.5.0. One bounded test runs independent Host, Receiver, and Connector children; proves signed HTTP event acceptance, exact event and claim replay across Receiver restarts, effect rejection, acknowledgement-response-loss convergence, and raw-token absence from Receiver files. No forced process termination, production shell, TLS, pairing, real Host-effect verifier, or Agent behavior is inferred |
...
| Challenge implementation | **RE-ENTRY CORE FOUNDATION IN PROGRESS; C1/C2/C3C LOCALLY VERIFIED** | Protocol, Host SDK, bounded Receiver authority, lease, effect acknowledgement, HTTP mapping, outbound client, and test-process isolation are locally verified; production process shells, Agent activation, selected app, deployment, demo, and submission closure remain open |
...
3. Complete [RECORE-001](../Development/RECORE-001-foundation.md) in bounded increments. The
   ...
   ADR-0010 transport plus test-process isolation are locally verified. Next test forced Receiver
   termination and ownership boundaries without changing Core authority or modifying MVP1 or MVP2.
