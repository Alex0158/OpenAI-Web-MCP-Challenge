...
**Phase:** Re-entry Core foundation is active under ADR-0006. The v0.1 protocol, Host SDK, ADR-0008 Receiver C1 authority, ADR-0009 Connector delivery C2, and ADR-0010 HTTP adapter and outbound Connector client C3b are locally verified on Node 24 and the current Node 26 runtime. C3b adds strict bounded Host-event, claim, and acknowledgement mapping plus a secure-origin, no-redirect, no-retry Connector client without changing Core authority. Independent Host, Receiver, and Connector processes remain unproved alongside production consent or pairing, a real Host-effect verifier, a supported Agent adapter, final-app selection, deployment, product value, and judge portability. Bounded mechanism feasibility remains passed through P0, H1, H2a, and the synthetic H2 service contract, while both standalone App Server Desktop-Browser joins remain failed in the current build.  
...
| Receiver HTTP and outbound Connector transport | **ADAPTER AND CLIENT LOCALLY VERIFIED; PROCESS PROOF OPEN** | ADR-0010 and RECORE-001 C3b verify three strict bounded JSON routes, canonical responses, redacted errors, secure-origin and response validation, no automatic retry, and explicit package subpaths. Independent Host/Receiver/Connector processes, TLS, pairing, real effects, and Agent behavior remain unproved |
...
| Re-entry Core transport C3 | **HTTP ADAPTER AND CLIENT LOCALLY VERIFIED; PROCESS PROOF OPEN** | ADR-0010 and RECORE-001 C3b pass 47 aggregate tests on Node 24.20.0 and Node 26.5.0, including exact HTTP mapping, bounds, redacted errors, secure origins, redirect rejection, timeout, stream interruption, and malformed-response rejection. No child-process, network restart, TLS, pairing, real Host-effect, or Agent behavior is inferred |
...
| Challenge implementation | **RE-ENTRY CORE FOUNDATION IN PROGRESS; C1/C2/C3B LOCALLY VERIFIED** | Protocol, Host SDK, bounded Receiver authority, lease, effect acknowledgement, HTTP mapping, and outbound client are locally verified; separate processes, Agent activation, selected app, deployment, demo, and submission closure remain open |
...
3. Complete [RECORE-001](../Development/RECORE-001-foundation.md) in bounded increments. The
   ...
   ADR-0010 HTTP adapter and outbound client are locally verified. Next prove separate Host,
   Receiver, and Connector processes without changing Core authority or modifying MVP1 or MVP2.
