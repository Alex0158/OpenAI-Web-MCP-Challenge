...
**Phase:** Re-entry Core foundation is active under ADR-0006. The v0.1 protocol, Host SDK, ADR-0008 Receiver C1 authority, ADR-0009 Connector delivery C2, ADR-0010 transport and forced-restart test-process isolation C3d, and ADR-0011 deterministic Agent Adapter contract C4b are locally verified on Node 24 and the current Node 26 runtime. ADR-0012 now specifies the smallest source-repository conformance/development profile that can exercise those ports outside `test/`; implementation evidence remains pending. Concurrent Receiver ownership, production process shells, consent or pairing, a real Host-effect verifier, a supported Agent adapter, private context-binding lifecycle, final-app selection, deployment, product value, and judge portability remain open. Bounded mechanism feasibility remains passed through P0, H1, H2a, and the synthetic H2 service contract, while both standalone App Server Desktop-Browser joins remain failed in the current build.  
...
| Domain-neutral conformance/development profile | **SPECIFIED** | ADR-0012 freezes one source-repository-only loopback profile with distinct Host, Receiver, and Connector children, deterministic Agent dispatch, separate synthetic Host effect, redacted output, and no production claim. Implementation remains pending |
...
| Challenge implementation | **RE-ENTRY CORE FOUNDATION IN PROGRESS; C1/C2/C3D/C4B LOCALLY VERIFIED** | Protocol, Host SDK, bounded Receiver authority, lease, effect acknowledgement, HTTP mapping, outbound client, forced-restart test-process isolation, and deterministic Agent Adapter contract are locally verified; production process shells, private context binding, real Agent activation, selected app, deployment, demo, and submission closure remain open |
...
3. Complete [RECORE-001](../Development/RECORE-001-foundation.md) in bounded increments. The
   ...
   ADR-0010 transport plus forced-restart test-process isolation and ADR-0011 deterministic Agent
   Adapter contract are locally verified. ADR-0012 now specifies the source-repository
   conformance/development profile; implement and verify that bounded profile next. Keep
