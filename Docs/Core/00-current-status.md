...
**Phase:** RECORE-001 through RECORE-004 are locally verified, including the RECORE-003 Program completion audit and the ADR-0013 Receiver Grant-control increment. RECORE-005 is specified as the next test-only separate-process fault-matrix increment; the broader Re-entry Core program remains active. The v0.1 protocol, Host SDK, Receiver C1 authority, authenticated same-subject Grant inspection and atomic revocation, Connector delivery C2, transport and forced-restart test-process isolation C3d, deterministic Agent Adapter C4b, source-repository conformance/development profile C6b, and bounded local quality baseline pass on Node 24 and the current Node 26 runtime. Grant control is verified only through the in-process Core authority and SQLite persistence boundary; no administration HTTP route or production control session is claimed. Private managed-context binding and the separate-process revocation, stale-worker, conflicting-effect, and mid-transaction failure matrix remain application-neutral gaps until directly verified. Production ownership, process shells, consent or pairing, credential custody, a real Host-effect verifier, a supported Agent adapter, final-app selection, deployment, product value, and judge portability remain decision-gated or open outside those bounded increments. Bounded mechanism feasibility remains passed through P0, H1, H2a, and the synthetic H2 service contract, while both standalone App Server Desktop-Browser joins remain failed in the current build.  
**Controlling decisions:** [ADR-0002](../Decisions/ADR-0002-separate-mechanism-from-demo-app.md), [ADR-0003](../Decisions/ADR-0003-freeze-p0-technical-validation-mvp.md), [ADR-0004](../Decisions/ADR-0004-separate-event-protocol-from-agent-transport.md), [ADR-0005](../Decisions/ADR-0005-run-additive-durable-enrollment-spike.md), [ADR-0006](../Decisions/ADR-0006-establish-reentry-core-development-baseline.md), [ADR-0007](../Decisions/ADR-0007-freeze-reentry-core-v0.1-contract-kernel.md), [ADR-0008](../Decisions/ADR-0008-freeze-receiver-authority-and-durable-reservation.md), [ADR-0009](../Decisions/ADR-0009-freeze-connector-lease-and-effect-acknowledgement.md), [ADR-0010](../Decisions/ADR-0010-freeze-receiver-http-and-connector-transport.md), [ADR-0011](../Decisions/ADR-0011-freeze-agent-adapter-activation-boundary.md), [ADR-0012](../Decisions/ADR-0012-freeze-domain-neutral-conformance-profile.md), and [ADR-0013](../Decisions/ADR-0013-freeze-receiver-grant-control-and-revocation.md)
...
| Re-entry Core identity and source baseline | **FOUNDATION LOCALLY VERIFIED; PROGRAM ACTIVE** | ADR-0006 establishes `reentry-core/`, freezes MVP1/MVP2 as references, and selects one Receiver Core with Cloud Receiver and outbound Local Connector process shells. RECORE-001 closes at its bounded local-evidence target without claiming production shells |
...
| Receiver HTTP and outbound Connector transport | **TRANSPORT AND FORCED-RESTART TEST ISOLATION LOCALLY VERIFIED** | ADR-0010 and RECORE-001 C3d verify three strict bounded JSON routes, a no-retry outbound client, and independent Host/Receiver/Connector children. The Host alone owns its private key, the Receiver alone opens SQLite, and the Connector uses only HTTP; three forced Receiver terminations preserve exact state. Production shells, TLS, pairing, real effects, and Agent behavior remain unproved |
...
| Re-entry Core transport C3 | **TRANSPORT AND FORCED-RESTART TEST ISOLATION LOCALLY VERIFIED** | ADR-0010 and RECORE-001 C3d pass 48 aggregate tests on Node 24.20.0 and Node 26.5.0. One bounded test runs independent Host, Receiver, and Connector children; proves signed HTTP event acceptance, exact event and claim replay after `SIGTERM`, effect rejection, acknowledgement-response-loss recovery through another forced termination, and raw-token absence from Receiver files. No mid-transaction crash injection, concurrent ownership, production shell, TLS, pairing, real Host-effect verifier, or Agent behavior is inferred |
...
| Re-entry Core quality and weight baseline | **LOCALLY VERIFIED, NON-SLA** | RECORE-002 runs bounded protocol, deterministic Agent Adapter, file-backed SQLite Receiver, and cold source-profile benchmarks on Node 24.20.0 and Node 26.5.0. The package retains zero runtime dependencies and 15 selected files; benchmark, conformance, and test sources remain excluded. This is same-machine regression evidence, not hosted throughput, version ranking, production latency, or an app budget |
| Re-entry Core Program completion audit | **LOCALLY VERIFIED; PROGRAM ACTIVE** | RECORE-003 maps every Definition of Done item to direct evidence and now records RECORE-004 as post-audit progress. Private managed-context binding, the remaining separate-process fault matrix, and final exact closure remain incomplete; production and selected-app work is not substituted for those Core gaps |
| Receiver Grant control | **LOCALLY VERIFIED IN PROCESS** | ADR-0013 and RECORE-004 verify one required Receiver authority port, authenticated same-subject inspection, Core-timestamped compare-and-set revocation, idempotent replay, real persisted event/delivery fencing, rollback, reopen, and token non-persistence on Node 24 and Node 26. No HTTP route, control UI/session, process separation, or production identity is proven |
...
| Challenge implementation | **RE-ENTRY CORE FOUNDATION AND LOCAL QUALITY BASELINE VERIFIED; PROGRAM ACTIVE** | Protocol, Host SDK, bounded Receiver authority, lease, effect acknowledgement, HTTP mapping, outbound client, forced-restart test-process isolation, deterministic Agent Adapter contract, non-production conformance profile, and local regression benchmarks are verified; production process shells, private context binding, real Agent activation, selected app, deployment, demo, and submission closure remain open |
...
3. Preserve [RECORE-001](../Development/RECORE-001-foundation.md) as the locally verified
   ...
   ledger, and [RECORE-004](../Development/RECORE-004-grant-control.md) as the locally verified
   Grant-control record. Execute the test-only
   [RECORE-005](../Development/RECORE-005-separate-process-fault-matrix.md) fault matrix without
   combining it with private managed-context binding, production hooks, or administration HTTP.
   Keep
