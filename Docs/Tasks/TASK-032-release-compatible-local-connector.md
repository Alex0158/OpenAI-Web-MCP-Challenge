...
- Dependencies: ADR-0037, ADR-0041, TASK-022, TASK-025, TASK-029, SDK-006, AUDIT-V2-007, and
  AUDIT-V2-012 in Core/09.
...
the current active-v2 Claim plus coordinated local integration at the exact intended source. Update
all active install guides to name only an artifact proven compatible with that contract.
...
Claim/full-chain scope; and registry integrity, `gitHead`, install guidance, and residual TASK-029
boundary are recorded. Publication requires separate owner authorization.

## 6. Reopen condition

Reopen if the registry tag, `gitHead`, package version, bundle contents, active Receiver response,
instruction validation, safety frame, clean-consumer result, or install guide diverges.
