...
| `Docs/Research/05–08` | SUPPORTING risk and mechanism research | 2–3 | Distributed seams, continuity alternatives, supported re-entry, P1 plan | Retain. Research 05 and Research 08 are partially superseded: use their risk and P1 test findings, not their old sequencing or pre-H2 enrollment status. |
...
| `Docs/Research/17-mvp1-mvp2-comparative-integration-review.md` | SUPPORTING named-commit branch review | 2–3 | MVP1/MVP2 mechanism comparison, selective-reuse map, and a local direct-queue observation | Retain as a snapshot of its named commits. Its selective-reuse findings remain useful, but it is not current-main status, an app selection, transport decision, complete-branch merge authorization, or self-contained Q0 evidence package. |
| `Docs/Research/18-receiver-queue-and-wake-adapter-architecture-review.md` | Partially superseded SUPPORTING architecture input | 2–3 | Host authority, Receiver delivery ledger, replaceable wake adapter, bounded Heartbeat, D4 priority, and the then-unresolved Browser/WebMCP join | Retain its authority separation, wake-adapter, and D4-priority analysis. Its prospective standalone App Server step is superseded by Research 19; use Core/00 for sequencing and Research 20 only for the conditional Workspace Agents gate. |
| `Docs/Research/19–20` | SUPPORTING platform-boundary evidence and audit | 2–3 | Failed standalone App Server Desktop joins and the conditional Workspace Agents hosted-runtime gate | Retain the exact current-build failure classifications and hosted-topology nonclaims. Do not infer the missing cold Browser/session precondition or a Workspace Agent Browser/WebMCP capability. |
| `Docs/Research/21–22` | SUPPORTING reconciled contributor analysis and provenance | 2–3 | Candidate Cloud Receiver/Local Connector topology and named-commit MVP2 selective-integration record | Retain as decision input and provenance. They do not select the topology, merge the runtime, or override app-first sequencing. |
...
| **MVP1 versus MVP2.** A bounded runtime review favors MVP2's product-shaped flow but finds authority, replay, durability, human-approval, and evidence gaps in its integrated core. The later contributor documents propose a Cloud Receiver/Local Connector topology with a last-mile-first sequence. | Keep MVP1 as the shared mechanism baseline and preserve MVP2 as a contributor reference. Main distills the topology as an unselected candidate and retains app selection plus a transport requirements profile as the prior gate. Import only exact assets after their app, protocol, or adapter decision; do not merge the complete branch. | [K37](02-high-value-register.md); [Research 17](../Research/17-mvp1-mvp2-comparative-integration-review.md); [Research 21](../Research/21-cloud-receiver-local-connector-candidate-topology.md); [Research 22](../Research/22-mvp2-selective-integration-provenance.md) |
| **80/80 versus 88/88 versus 114/114 versus 118/118 tests.** Earlier MVP and H2 summaries reported their then-current totals; later lifecycle and evidence controls raised the full suite. | Keep historical counts in their evidence context; use 118/118 for the current full suite, 114/114 for the D4-hardening checkpoint, 88/88 for the H2-era full suite, and 30/30 for focused H2. | [current status](../Core/00-current-status.md), [H2 verdict](../../mvp/evidence/h2-durable-enrollment-service-contract-2026-08-30-verdict.md), [D4 inconclusive attempt](../../mvp/evidence/d4-h2b-first-formal-no-event-inconclusive-2026-08-30.md) |
| **37, 23, 59, 88, 114, and 118 test counts.** These numbers refer to different scopes: P0 component/contract tests, frozen clean-run snapshots, combined P0+H1 history, the H2-era full suite, the D4-hardening checkpoint, and the current suite with App Server join-probe evidence controls. | Never present them as one cumulative claim. Name the suite and date whenever a count is reported. | [P0 evidence](../../mvp/evidence/README.md), [current status](../Core/00-current-status.md) |
| **Direct event wake versus scheduled pull.** P0 used a private current-build Desktop bridge; H0b/H1 used same-task scheduled turns. | Report them as separate adapter paths. Event acceptance is not the same as direct wake, and Scheduled Heartbeat is a bounded fallback adapter rather than event authority, the core mechanism, or a production transport. | [ADR-0004](../Decisions/ADR-0004-separate-event-protocol-from-agent-transport.md), [Research 07](../Research/07-supported-reentry-transport-and-heartbeat-spike.md), [Research 18](../Research/18-receiver-queue-and-wake-adapter-architecture-review.md) |
| **D4 priority versus the supported transport join.** Earlier plans made full Desktop restart the next app-neutral gate; its first arm was inconclusive and would prove only one local relaunch topology. Both later standalone App Server alternatives also failed on the tested current build: the cold arm's Browser selector returned `iab-unavailable` before page access without identifying the absent Browser or session precondition, and exact warm resume returned an active-writer rejection. | Freeze D4 as optional compatibility evidence, remove both tested standalone App Server Desktop joins from current selection unless a materially different supported contract or topology appears, and make app selection the current gate. Evaluate a distinct published Workspace Agents hosted topology only when entitlement and selected-app requirements justify it, with its own Browser/WebMCP proof. | [current status](../Core/00-current-status.md), [cold join evidence](../../mvp/evidence/app-server-browser-join-probe-2026-08-30.json), [warm join evidence](../../mvp/evidence/app-server-browser-warm-join-probe-2026-08-30.json), [Research 10](../Research/10-post-h1-unknowns-and-validation-roadmap.md), [Research 18](../Research/18-receiver-queue-and-wake-adapter-architecture-review.md) |
...
### High — preserve before app selection

1. Preserve the two failed standalone App Server Browser/WebMCP probes as current-build evidence:
   the cold Browser selector returned `iab-unavailable` before page access without identifying
   the absent Browser or session precondition, and exact warm resume returned an active-writer
   rejection for the supplied task. The public warm artifact does not identify the writer owner.
   Do not repeat the same standalone Desktop route without a materially different
   supported platform contract.
2. Keep D4/H2b frozen as optional compatibility evidence. Rerun it only if a selected local
   connector or relaunch topology makes same-machine Desktop restart recovery material.
3. Keep Scheduled Heartbeat as a bounded fallback experiment, not the core mechanism or a
...
2. If entitlement and selected-app requirements justify a hosted topology, evaluate a published
   Workspace Agent with its own Browser/WebMCP evidence boundary; do not treat it as continuation
   of an arbitrary local Desktop task.
