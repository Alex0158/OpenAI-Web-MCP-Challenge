...
test("local preview process pairs a Connector and closes both stores", async (t) => {
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "cloud-receiver-preview-process-"));
  const databasePath = join(temporaryDirectory, "receiver.sqlite");
  const pairingDatabasePath = join(temporaryDirectory, "pairing.sqlite");
  const mainPath = resolve(import.meta.dirname, "../src/main.mjs");
  const compositionPath = resolve(import.meta.dirname, "../src/local-preview-composition.mjs");
  const hostApiKey = "host-preview-api-key";
  const connectorSecret = "connector-preview-secret";
  let child;
  t.after(async () => {
    if (child && child.exitCode === null && child.signalCode === null) {
      child.kill("SIGKILL");
  ...
  child = spawn(process.execPath, [mainPath], {
    env: {
      ...
      CLOUD_RECEIVER_COMPOSITION_MODULE: compositionPath,
      CLOUD_RECEIVER_DATABASE_PATH: databasePath,
      CLOUD_RECEIVER_PAIRING_DATABASE_PATH: pairingDatabasePath,
      CLOUD_RECEIVER_HOST_API_KEY: hostApiKey,
      CLOUD_RECEIVER_CONNECTOR_TOKEN_SECRET: connectorSecret,
      CLOUD_RECEIVER_HOST: "127.0.0.1",
      CLOUD_RECEIVER_PORT: "0",
    },
    stdio: ["ignore", "pipe", "pipe"],
  ...
  const started = await waitForLogEvent(child, () => stdout, "cloud_receiver_started");
  const origin = `http://127.0.0.1:${started.port}`;
  const pairing = await postJson(origin, "/v0.1/pairing-sessions", {
    headers: { Authorization: `Bearer ${hostApiKey}` },
    body: { host_subject_ref: "host_user_process" },
  });
  assert.equal(pairing.response.status, 201);
  const approval = await postJson(origin, "/v0.1/pairing-sessions/approve", {
    body: { user_code: pairing.body.user_code },
  });
  assert.equal(approval.response.status, 200);
  const claim = await postJson(origin, "/v0.1/pairing-sessions/claim", {
    body: { user_code: pairing.body.user_code },
  });
  assert.equal(claim.response.status, 200);
  const credentials = await postJson(origin, "/v0.1/pairing-sessions/poll", {
  ...
  assert.equal(stderr, "");
  assert.ok((await stat(databasePath)).isFile());
  assert.ok((await stat(pairingDatabasePath)).isFile());
  for (const filename of [databasePath, pairingDatabasePath]) {
    const persisted = await readFile(filename);
    assert.equal(persisted.includes(Buffer.from(hostApiKey)), false);
    assert.equal(persisted.includes(Buffer.from(connectorSecret)), false);
  }
});
...
async function postJson(origin, path, { headers = {}, body }) {
