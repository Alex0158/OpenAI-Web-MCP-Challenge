...
- Next gate: No further gate remains for this local presentation scope; hosted package compatibility, genuine WebMCP/Agent behavior, external delivery, and final judge reproduction remain separate CP-18 gates.
...
Give a judge a clear, light, and self-contained route from Local Connector installation and signed Manifest consent to the Sleepless Kingdom game path, the Re-entry vision, the supplied video, and two linked mini-application surfaces.
...
- Success: The page contains image-led, keyboard-accessible entries for RightSpot and `https://reentry-sdk-playground.vercel.app/`, with the SDK Playground copy identifying it as another mini application that tests Re-entry.
...
- Non-goals: Adding gameplay or state transitions, changing the WebMCP tool contract, changing authentication or server behavior, claiming hosted Agent wake or genuine WebMCP execution, modifying RightSpot, modifying Re-entry Core/Host SDK/Local Connector/Cloud Receiver, or deploying external services.
...
- Out of scope: `app/page.tsx`, `app/layout.tsx`, `src/server/`, `src/shared/`, persistence, worker logic, HTTP/WebSocket/WebMCP contracts, `reentry-core/`, `mvp/`, RightSpot source, the Host SDK, Local Connector, Cloud Receiver, Clerk, Railway, Vercel, and external deployment configuration.
...
- Verified: The existing judge-guide route is static, has the accepted page-tool names, and already has a local RightSpot image entry.
...
- Unknown: Hosted deployment of the revised route, final package compatibility, genuine browser WebMCP discovery/invocation, external Receiver/Connector delivery, Agent wake, and final judge reproduction.
...
- Added image-led entries for RightSpot and the Re-entry SDK Playground at `https://reentry-sdk-playground.vercel.app/`, using a project-owned SVG preview for the latter.
...
- Browser readback: passed at `1440 x 1000` and `390 x 844`; document and client widths matched, the YouTube iframe and both mini-application images loaded, and no horizontal overflow was reported.
...
- Claim limit: This closes the local judge-facing presentation and tutorial surfaces only. It does not prove hosted package compatibility, genuine WebMCP invocation, Agent wake, external Receiver/Connector delivery, or final judge reproduction.
