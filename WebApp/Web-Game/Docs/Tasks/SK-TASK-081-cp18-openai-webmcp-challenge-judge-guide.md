...
- Current increment: The static `OpenAI-WebMCP-Challenge-Judge-Guide` route is implemented as a reviewer-facing explanation of the Sleepless Kingdom causal story, Re-entry path, page tools, and human consequence boundary.
...
observe in the game, how a meaningful event becomes a bounded Agent re-entry, which page tools are
available, and where human/server authority remains.
...
- Success: The page remains readable at the tested wide and narrow viewports with no horizontal
  overflow, no remote font dependency, and keyboard-visible link focus states.
...
- In scope: `app/OpenAI-WebMCP-Challenge-Judge-Guide/page.tsx`, its CSS module, and this task record.
...
- Allowed actions: Edit the two new route files, run focused build/type/browser checks, and record
  the result. Do not stage, commit, push, deploy, or contact external parties as part of this task.
...
- Minimum verification: `npm run typecheck`; `npm run build`; local `200`/title/content readback;
  disposable wide/narrow browser readback; and whitespace/diff checks for the new files.
- Closure target: `integrated` for the named local reviewer-facing presentation surface only.
- Rollback or remediation: Remove only the two route files and this task record if the route blocks
  the build, overflows at the tested viewport, or introduces an unsupported product claim. Preserve
  all unrelated dirty and untracked work.
...
- Browser readback: passed at `1440 x 1000` and `390 x 844`; document/client widths were equal in
  both contexts and the page rendered without horizontal overflow.
