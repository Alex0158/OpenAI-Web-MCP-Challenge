...
| S14-B External handoff and Thread backpressure | No game-side Receiver/Connector module; external ownership is outside this tree | Bind the port to Eddy's exact versioned transport, acknowledgement, lease/retry, idempotency, and active-Thread contract | R14-06 and R14-09 | CP-14 delivery trace and CP-16 causal evidence | Versioned external handoff required for live integration; acknowledgement never equals game command success |
...
| S16-A Local vertical slice | `src/server/fixture-session.ts`, `src/server/entrypoint.ts`, `src/client/live-game-projection.tsx`, CP-14/15 evidence | Execute the reset, two-player, browser-close, loss, signal, reread, recall, restart, burst, and trace runbook | S16-01 through S16-07 | CP-15 matrix, CP-12 projection/reconnect checks, and [`SK-EVID-051`](../Evidence/SK-EVID-051-cp16-independent-browser-context-capability-probe.md) | CP-15 closure plus two genuine browser contexts; the current IAB two-tab limitation is verified but remains open for level 5 |
| S17-A Hosted continuity | `src/server/entrypoint.ts`, `src/server/runtime.ts`, `src/server/health.ts`, [`06-operations-and-hosting.md`](06-operations-and-hosting.md) | Map the one-process local contract to a selected host, durable store, supervisor, URL, TLS, health, and rollback path | H17-01 through H17-08 | CP-16 trace, build, typecheck, process/restart, and host readbacks | CP-16 closure and explicit host decision; no provider-by-guess, ephemeral storage, or serverless timer |
...
5. Run the CP-15 row-complete matrix and record a Verification Budget. A green local suite cannot
   upgrade an external, hosted, two-session, or judge claim.
6. Execute the CP-16 local rehearsal with a fresh file-backed fixture, explicit worker/world-time
   advances, two independent contexts, browser absence, restart, and a timestamped redacted trace.
7. Only after CP-16 closure choose a host and execute the CP-17 deployment rehearsal. CP-18 consumes
   the exact hosted evidence and cannot repair an earlier gap.
