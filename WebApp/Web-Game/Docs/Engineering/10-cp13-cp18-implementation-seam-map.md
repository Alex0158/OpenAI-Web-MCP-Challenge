...
| S14-B External handoff and Thread backpressure | No game-side Receiver/Connector module; external ownership is outside this tree | [`SK-TASK-076`](../Tasks/SK-TASK-076-cp14-cloud-receiver-v2-game-adaptation.md) binds one Game signal to Eddy's exact signed Host Event ingress, while Eddy retains Connector claim/lease, fresh activation, Thread backpressure, and effect ACK ownership | R14-06 and R14-09 | CP-14 delivery trace and CP-16 causal evidence, plus [`Validation/89`](../Validation/89-cp14-cloud-receiver-v2-adaptation-cross-functional-audit.md) | Versioned external handoff required; Cloud `202` means queue acceptance only; Game must not claim/ACK the Cloud delivery or treat acknowledgement as game command success |
...
| S17-A Hosted continuity | `src/server/entrypoint.ts`, `src/server/runtime.ts`, `src/server/health.ts`, [`06-operations-and-hosting.md`](06-operations-and-hosting.md) | **Preflight tracked under [`SK-TASK-077`](../Tasks/SK-TASK-077-cp17-host-decision-and-deployment-preflight.md):** reconcile production bootstrap, identity, host/store, URL/TLS, health, WebSocket, supervision, and rollback before mapping the one-process contract to a selected host | H17-01 through H17-08 | CP-16 trace, Node 24 build/typecheck, production-like bootstrap/identity checks, process/restart, and host readbacks | Explicit host/topology ADR and production identity/bootstrap gate; no provider-by-guess, fixture deployment, ephemeral storage, or serverless timer |
...
8. Run [`SK-TASK-077`](../Tasks/SK-TASK-077-cp17-host-decision-and-deployment-preflight.md) as an
   early CP-17 preparation stream: audit the production start/configuration, compare host facts, and
   prove the local production-like bootstrap/identity prerequisites. Only after an explicit host/topology
   ADR and those prerequisites pass, execute the CP-17 deployment rehearsal. CP-18 consumes the exact
   hosted evidence and cannot repair an earlier gap.
