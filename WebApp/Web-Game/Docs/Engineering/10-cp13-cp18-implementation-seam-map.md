...
**Status:** CP-13 local page implementation, canonical four-read capability, and the CP-14 game-side local-stub plus standing transport contract are verified for named local scopes; live Receiver/Connector delivery, same-task admission, independent browser contexts, hosted continuity, and judge reproduction remain gated
...
**Source topology note (2026-09-04):** Eddy's Game-facing Core, Host SDK, and Local Connector source is already in the outer main history; no branch merge is pending. The Game tree still contains no Receiver runtime by design. The Cloud Receiver backend is a separate saas-boilerplate deployment, and CP-14 now verifies the Game adapter against that selected runtime.
...
- The additive CP-14 standing mapper (`src/server/standing-reentry-transport.ts`) now persists
  schema-9 Event context and sequence through `PersistenceStore`, resolves only a server-side public
  binding, and maps a valid Receiver response to the narrow `receiver_queue_accepted` boundary. It
  has no Connector lease, task locator, timer, or page authority.
- The CP-14 `ReentryDeliveryRunner` is wired as one optional worker lifecycle observer in
  `src/server/entrypoint.ts`: startup and completed authoritative world boundaries coalesce into one
  in-flight `pumpOnce`, 100 ms interpolation ticks are ignored, failures remain observable, and
  shutdown drains it before the worker/store closes. It has no durable authority, remote retry loop,
  second queue, or world clock.
...
- The game tree contains no Receiver runtime by design. The outer main history already contains Eddy's Game-facing Core, Host SDK, and Local Connector source; CP-14's live gate is the Game adapter plus compatibility readback against the separately deployed Receiver backend, not another branch merge.
...
| S14-A Event to delivery slot | `src/server/persistence/store.ts` and `src/server/persistence/types.ts` (`signalSlot`, `outboxDelivery`, claim/ack/retry/reject) | **Implemented locally under `SK-TASK-062`:** `ReentryDeliveryPort`/`pumpOnce` selects one existing durable record and maps one coalesced envelope; `ReentryDeliveryRunner` attaches it to worker startup/advance with single-flight coalescing and shutdown drain | R14-01 through R14-05 | CP-05 persistence, CP-11 cargo-loss, CP-13 page reread, [`SK-EVID-050`](../Evidence/SK-EVID-050-cp14-game-side-local-stub-delivery-port-runtime-verification.md), and [`SK-EVID-079`](../Evidence/SK-EVID-079-cp14-reentry-delivery-runner-runtime-verification.md) | Local stub/runner evidence is verified; no second queue, timer, worker, world clock, or identity resolver; live external binding and remote wake policy remain gated |
| S14-B External handoff and Thread backpressure | `src/server/standing-reentry-transport.ts` plus Game persistence; Receiver/Connector remain separate repositories and authorities | [`SK-TASK-076`](../Tasks/SK-TASK-076-cp14-cloud-receiver-v2-game-adaptation.md) and [`ADR-GAME-0039`](../Decisions/ADR-GAME-0039-cp14-bound-task-notification-adoption.md) bind ordered Game signals to protocol-v0.2 standing Event ingress and the same-task notification target; the Connector/Adapter owns private task binding, notification handoff, one-active scheduling, and Thread backpressure | R14-06 and R14-09 | CP-14 same-task two-signal trace and CP-16 causal evidence, plus [`Validation/89`](../Validation/89-cp14-cloud-receiver-v2-adaptation-cross-functional-audit.md) and outer RECORE-007 | Exact v0.2 compatibility readback against the reviewed SDK/Connector source and selected Receiver backend is required; no branch merge remains pending; queue acceptance, notification handoff, Agent wake, optional action, and Game effect remain separate; Game retains a signal on retryable transport/backpressure and never handles the Cloud lease |
...
4. After positive canonical CP-13 evidence, the game-side CP-14 port is implemented and verified under
   ...
   binding, lease identity, and acknowledgement semantics intact; use the merged source baseline and
   keep live integration against the selected Receiver deployment.
5. Before any live external adapter code, process [`SK-TASK-076`](../Tasks/SK-TASK-076-cp14-cloud-receiver-v2-game-adaptation.md)
   against the reviewed SDK/Core/Connector source and selected Receiver Preview, using [`ADR-GAME-0039`](../Decisions/ADR-GAME-0039-cp14-bound-task-notification-adoption.md)
   as the selected-product boundary. Keep the Game publication lease separate from the Cloud
   Connector lease, use a server-only binding/workflow/sequence map, and record `202` as queue
   acceptance only. Same-task notification handoff, Agent wake, page action, and optional Game effect
   require separate evidence. Do not use the deprecated `runtime/cloud-receiver/` or guess the
   consent route.
...
| Delivery | One pending/in-flight signal per shelter/binding, coalesced eligible context, explicit publication/notification-handoff outcomes, and no per-event Thread wake; Agent action is optional and separate |
...
This map is complete for documentation-level routing. CP-13's local page implementation and canonical four-read capability are verified, and the CP-14 game-side local-stub port, additive standing transport contract, and worker delivery runner are verified under `SK-EVID-050`, `SK-EVID-078`, and `SK-EVID-079`,
but this map does not close CP-14 external delivery or same-task admission, CP-15 runtime
