...
**Cloud Receiver adaptation:** [`SK-TASK-076`](Tasks/SK-TASK-076-cp14-cloud-receiver-v2-game-adaptation.md) is `pending` under [`Validation/89`](Validation/89-cp14-cloud-receiver-v2-adaptation-cross-functional-audit.md). The Game is treated as the Host and will publish one server-signed Event to Eddy's exact v2 Receiver; Cloud `202` is queue acceptance only, while Connector claim, fresh Codex activation, page action, and effect ACK remain separate gates. No external adapter code is admitted until Eddy supplies the exact v2 implementation/package/endpoint/test handoff, consent/binding/session mapping, and effect-authority boundary.  
...
**Hosted deployment preflight:** [`SK-TASK-077`](Tasks/SK-TASK-077-cp17-host-decision-and-deployment-preflight.md) remains `in_progress` with audit [`Validation/90`](Validation/90-cp17-host-decision-and-deployment-preflight-cross-functional-audit.md). [`SK-TASK-078`](Tasks/SK-TASK-078-cp17-production-identity-and-hosted-admission.md) is `in_progress`: the local production-like path now has idempotent world bootstrap, Clerk subject-to-player/shelter scope, generic bootstrap, HTTP command, WebSocket admission, and the minimal invite-only Clerk client gate under focused Node 24 tests; Railway resources and non-secret service settings are ready for source deployment. Clerk credential/subject setup, Game deployment, real session issuance, public runtime, restart, backup/restore, and hosted evidence remain gated.
...
the next action is to supply the two Clerk Production subject bindings, deploy the exact Game source,
and execute the remaining rehearsal under [`SK-TASK-077`](Tasks/SK-TASK-077-cp17-host-decision-and-deployment-preflight.md) and
[`SK-TASK-078`](Tasks/SK-TASK-078-cp17-production-identity-and-hosted-admission.md), while
[`SK-TASK-076`](Tasks/SK-TASK-076-cp14-cloud-receiver-v2-game-adaptation.md) awaits Eddy's exact external
handoff. Neither stream may claim hosted continuity or live Agent delivery until its own evidence gate passes.
