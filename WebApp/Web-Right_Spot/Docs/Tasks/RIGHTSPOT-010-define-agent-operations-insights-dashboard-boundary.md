...
- ADR-RS-0013 accepts tenant Favourites and agent listing-interest aggregates, and `RIGHTSPOT-020` is
  closed within that bounded implementation outcome. `RIGHTSPOT-009` is a reviewed and deferred
  proposal for structured Information Requests and contact preferences. Favourite data is therefore
  available as a current bounded signal, while the proposal must remain valid without Information
  Request data or any other deferred capability.
...
- RightSpot `RIGHTSPOT-009` and the closed `RIGHTSPOT-020` task files, without treating deferred
  Information Requests or any future unimplemented signals as available Operations data
