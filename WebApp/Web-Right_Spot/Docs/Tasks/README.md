...
- [`RIGHTSPOT-050 — Withhold stale Tenant mutation actions after failed conflict recovery`](RIGHTSPOT-050-withhold-stale-tenant-actions-after-conflict-recovery.md)
- [`RIGHTSPOT-051 — Observe Tenant Search WebMCP registration failures`](RIGHTSPOT-051-observe-tenant-webmcp-registration-failures.md)
...
`RIGHTSPOT-049` is `CLOSED_VERIFIED`, a non-blocking P2 client compatibility repair for `F-23/F-24`
and the same response-truth boundary: filtered successful responses must not inherit unfiltered metadata
...
012 delta audit reproduced the scalar mismatch. `RS-WO-049-03` completed against the corrected
contract and fresh independent verification passed against unchanged RightSpot projection checkpoint
`9994f4e` after separately recorded Web-Game ref incidents. The task still owns only `src/ui/tenant/tenant-api.ts` and `tests/ui/tenant-api.test.ts`; it
...
`RIGHTSPOT-049` has completed and independently verified the bounded RS-WO-049-03 scalar-correlation
...
URL policy blocked the temporary proxy. `RIGHTSPOT-051` is now `READY_FOR_INDEPENDENT_VERIFICATION`
after supporting Builder Socrates returned the exact three-path candidate under
`gpt-5.6-sol + medium`; it is not yet independently verified, integrated, or closed. Main has
recorded the candidate hashes and the shared-index mixed-scope commit hazard in the Task and
validation ledger. The Agent-side registration-observability portion of the same `F-26` finding
