...
| Evidence | **FOUNDATION + DOMAIN CORE + PERSISTENCE/APPLICATION BOUNDARY + DISCOVERY API VERIFIED** | The bounded discovery candidate passed independent verification at source snapshot `bc3bc42`, including foundation `6/6`, focused `35/35`, build, API smoke, role/error/privacy checks, and no-mutation; ADR-RS-0008 now freezes the ordinary workflow HTTP/DTO boundary; workflow transport, shell, UI/UX, browser, deployment, and complete product-flow evidence remain open |
