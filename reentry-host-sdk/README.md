...
The npm registry currently serves `@4xeoz/re-entry-sdk@0.3.2`, including the `createReentry()`
simple facade shown below:

```sh
npm install @4xeoz/re-entry-sdk
# For local simple-facade verification instead:
npm install /absolute/path/to/<checkout>/reentry-host-sdk
```
...
The package name in this checkout is `@4xeoz/re-entry-sdk`. Use the included sample or an
explicit local file dependency when testing local source changes; do not copy SDK source into the
Host application or edit the bundled Core dependency by hand.
...
3. Put these server-only values in `reentry-host-sdk/app/.env.local`:
...
reentry-host-sdk/README.md and the example under reentry-host-sdk/app. Do not target the deprecated
