...
> **Cloud Receiver dependency notice — 2026-09-03:** This reduced testing branch contains the
> Connector package, not a Receiver implementation. The former `runtime/cloud-receiver/` service is
> deprecated and is intentionally excluded. The `re-entry-weld.vercel.app` hostname now serves the
> active frontend and is not the Receiver API origin. This Connector remains a reusable/local
> preview surface and defaults to the accepted active-v2 preview Receiver
> `https://cloud-receiver-delta.vercel.app`; pass `--receiver` for another accepted Receiver. This
> preview default is not a production deployment.
...
```sh
npm install --global /absolute/path/to/<checkout>/reentry-local-connector
re-entry install
```
...
directory—not from a checked-out `reentry-local-connector` package directory. npm can treat that
...
`/absolute/path/to/<checkout>/reentry-local-connector`; do not use registry
...
cd <checkout>/reentry-local-connector
npm install
npm run verify
```
